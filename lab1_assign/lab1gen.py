...
#         # number_of_modules = MaxNumberOfModules
...
#
#     fout.close()
def generate_input(path_prefix):
    for j in range(1, 20):  # Generating 20 input files
        file_name = f'input-{j}'  # Construct file name
        with open(file_name, 'w') as fout:  # Open file for writing
            symbol_table = {}
            number_of_modules = random.randint(1, MaxNumberOfModules)
            NumberOfModules = number_of_modules
            instr_count = 0
